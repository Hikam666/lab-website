...
// 1. Ambil ID fasilitas dari parameter URL
$id = $_GET['id'] ?? null;
...
if (isset($_POST['submit'])) {
    ...
    // Untuk mengisi ulang form jika ada error
    $data['nama']      = $nama; 
    ...
    <form action="" method="POST" enctype="multipart/form-data">
        ...
        <div class="mb-3">
            ...
            <select name="status" class="form-control" required 
                    <?= (function_exists('isAdmin') && !isAdmin()) ? 'disabled' : '' ?>>
                <?php $current_status = $data['status'] ?? 'draft'; ?>
                <option value="draft"      <?= ($current_status == 'draft'      ? 'selected' : '') ?>>Draft</option>
                <option value="diajukan"   <?= ($current_status == 'diajukan'   ? 'selected' : '') ?>>Diajukan</option>
                ...
                <option value="arsip"      <?= ($current_status == 'arsip'      ? 'selected' : '') ?>>Arsip</option>
            </select>
            <?php if (! (function_exists('isAdmin') && isAdmin())): ?>
                <small class="text-muted">
                    *Karena Anda operator, status akan otomatis menjadi <strong>diajukan</strong> setelah disimpan, terlepas dari pilihan di atas.
                </small>
            <?php endif; ?>
        </div>
